// Standalone pi app verification: no selector, no hub — just the app's own
// wire. create → prompt → ride the cursor to the real reply. Also asserts
// the pi-native duplication: the turn must materialize a session JSONL under
// ~/.pi/agent/sessions/<encoded-cwd>/ while the DSH log serves the readback.
import { existsSync, readdirSync, readFileSync, statSync } from "node:fs";
import { homedir } from "node:os";
import { join } from "node:path";

const jar = readFileSync(process.env.AW_JAR ?? "/tmp/aw-pi-app.jar", "utf8");
const line = jar.split("\n").filter((l) => l && (!l.startsWith("#") || l.startsWith("#HttpOnly_"))).pop();
const cookie = line.split("\t").slice(-2).join("=");
const base = process.env.AW_BASE ?? "http://192.168.31.130:4987";
const CWD = process.env.AW_CWD ?? "/home/u1/workspaces/superd/.scratch/aw-pi";
let n = 0;
...
const piSessionsDirsBefore = new Set(
  existsSync(join(homedir(), ".pi/agent/sessions"))
    ? readdirSync(join(homedir(), ".pi/agent/sessions"))
    : [],
);

console.log("providers:", JSON.stringify(await rpc("llm/listProviders", { args: {} })));
console.log("presets:", JSON.stringify(await rpc("agentPresets/list", { args: {} })).slice(0, 200));

const created = await rpc("session/create", { args: { request: { cwd: CWD } } });
console.log("create:", JSON.stringify(created).slice(0, 160));
const sessionId = created?.sessionId;
if (!sessionId) { console.error("no session"); process.exit(1); }

// Lazy iron law: creating a Dash session must not touch ~/.pi.
const encodedCwd = "--" + CWD.replaceAll("/", "-").replace(/^-/, "") + "--";
const piDir = join(homedir(), ".pi/agent/sessions", encodedCwd);
const lazyLeak = existsSync(piDir) && readdirSync(piDir).some((f) => f.endsWith(".jsonl") && !piSessionsDirsBefore.size);
console.log("lazy check: pi session dir before first prompt →", existsSync(piDir) ? readdirSync(piDir) : "(absent)");
...
while (Date.now() < deadline) {
  ...
  if (types.includes("assistant/message")) {
    ...
    console.log("usage on assistant/message:", JSON.stringify(usage ?? null));
    // Duplication proof: pi's native session file must exist for this cwd.
    if (existsSync(piDir)) {
      const files = readdirSync(piDir).filter((f) => f.endsWith(".jsonl"));
      const fresh = files.map((f) => join(piDir, f)).filter((f) => statSync(f).size > 0);
      console.log("pi-native session files:", files.length ? JSON.stringify(files) : "(none)");
      console.log(fresh.length > 0 ? "DUPLICATION OK — pi native copy exists alongside the DSH log" : "pi session file empty");
    } else {
      console.log("MISSING: no ~/.pi session dir for this cwd —", piDir);
    }
