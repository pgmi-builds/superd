/**
 * OmpAgent — the Dash `Agent` shim over one live OMP SDK sidecar session.
 *
...
export class OmpAgent implements Agent {
  ...
  #mirroredTitle: string | undefined;
   ...
   * creation value.
   */
  async #mirrorOmpTitle(): Promise<void> {
    if (!this.#rpc.spawned) return;
    try {
      const state = await this.#rpc.getState();
      this.#applyOmpTitle((state as { title?: unknown }).title);
    } catch {
   ...
   */
  #applyOmpTitle(title: unknown): void {
    if (typeof title !== "string" || title.trim() === "") return;
    if (title === this.#mirroredTitle) return;
