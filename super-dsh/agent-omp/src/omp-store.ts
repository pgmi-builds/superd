...
export const OMP_SESSIONS_ROOT =
  process.env.OMP_SESSIONS_ROOT ?? join(OMP_NATIVE_HOME, "agent", "sessions");
 ...
 */
export function readOmpModelRolesFromConfig(): Record<string, string> | undefined {
  let text: string;
  try {
  ...
  for (const line of text.split("\n")) {
    ...
    if (match !== null) roles[match[1]] = match[2];
  }
