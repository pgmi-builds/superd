#!/usr/bin/env bash
# start-claude-app.sh — the claude adapter as a STANDALONE app (no ctx0 selector,
# no hub, no spawned world): one dsh process whose composition IS the adapter
# bundle (claude-provider + native-row disables + permission table + the pinned
# browse directory picker).
# LAN via a socat relay bound to the LAN IP only (never 0.0.0.0).
set -euo pipefail
PORT="${AW_APP_PORT:-4989}"
UNIT="aw-claude-app-${PORT}-test"
RELAY_UNIT="aw-claude-app-${PORT}-relay"
LAN_HOST="${SUPERD_LAN_HOST:-192.168.31.130}"
MAIN_REPO="$HOME/workspaces/superd"
WT="$MAIN_REPO"   # main checkout: the adapter lives at super-dsh/agent-claude
LOG="$MAIN_REPO/.scratch/aw-claude-app-${PORT}.log"
...
# nested claude home (reads ~/.claude READ-ONLY, one-way valve)
DSH_HOME="$HOME_DIR" "$NODE_BIN" "$WT/super-dsh/agent-claude/scripts/setup-claude-home.mjs"
  ...
  "dsh": {
    "profile": {
      "bundles": [
        ...
        "@pgmi-builds/agent-adapter-claude"
      ]
    }
  }
}
...
# Standalone claude app: a REAL user-facing listener (loopback; LAN only via
# the socat relay). The provider rows (claude-provider + native-row disables +
...
# @pgmi-builds/agent-adapter-claude lives in super-dsh/agent-claude)
ln -sfn "$WT/super-dsh/agent-claude" "$PGMB/agent-adapter-claude"
...
systemd-run --user --unit="$UNIT" \
  --property=WorkingDirectory="$WT/super-dsh/agent-claude" \
  ...
  --property=Environment="CLAUDE_TRACE=${CLAUDE_TRACE:-0}" \
...
sleep 1
curl -s -c /tmp/aw-claude-app.jar -L -o /dev/null -w "LAN follow → %{http_code}\n" "http://${LAN_HOST}:${PORT}/?token=${TOKEN}" || true
