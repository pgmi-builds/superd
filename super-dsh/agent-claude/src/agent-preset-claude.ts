/**
 * SingleClaudePresetRoster — the profile's `agentPresets` service.
 *
 * Claude is single-mode: the Claude Code CLI session drives its own tools and
 * permission surface, so no Dash-side preset composition exists to choose
 ...
 * rather than "one mode" — the hand-test finding ("claude preset degraded").
 * This service publishes exactly one non-authorable preset, `Claude`, whose
 * composition is deliberately empty — mounting it composes nothing, exactly
 * the rosterless behavior the provider runs on.
 *
 * REGISTRATION LESSON (ported verbatim from the omp/pi bridges): this service
 * must be constructed on the PROVIDER's own top-level fiber, never under
 * `ctx.plugin(...)` — a child fiber hides its @Remote routes from the API
 * gateway's root-level remote enumeration and every agentPresets/* call
...
const CLAUDE_PRESET: AgentPreset = Object.freeze({
  id: "claude",
  trust: "system",
  path: "",
  name: "Claude",
...
export class SingleClaudePresetRoster extends TypertRemoteService {
  ...
  async resolve(id?: string): Promise<AgentPreset> {
    if (id === undefined || id === CLAUDE_PRESET.id) return CLAUDE_PRESET;
  ...
  async remoteList(): Promise<AgentPresetRoster> {
    if (process.env.CLAUDE_TRACE === "1") process.stderr.write("[claude-roster] remoteList INVOKED\n");
    return {
      presets: [
        {
          id: CLAUDE_PRESET.id,
  ...
  async copy(_from: string, id: string, _name?: string): Promise<void> {
    throw new RemoteError(
      ...
      `agent-presets: preset "${id}" cannot be written: the Claude roster is fixed and ships exactly one preset`,
      { agentPreset: id, reason: "the Claude roster is fixed and ships exactly one preset" },
    );
  }
