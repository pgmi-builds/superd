/**
 * Minimal in-memory inbox for the Codex agent shim.
 *
 ...
 * (`ReactLoopInbox`) lives inside the (disabled) `dsh-agent-loop`. Codex owns
 * its own transcript and the adapter delivers messages directly to the Codex
 * client (`CodexAgent.#deliver`), so this inbox is never queued into — it
