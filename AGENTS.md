...
- **版本台账（动态；格式 = `值 [updated 日期, provenance]`，尾部 `[]` 为下次更新位）**：
  - **dev 时点的上游版本**（`upstream/deepseek-harness` checkout，本仓唯一测试基准）：`dsh-v0.1.5-rc.2` [updated 2026-09-14，**物理 checkout**：detached 到该 tag、浅克隆（无完整 git 历史、无 worktree 漂移），即 dev 那一刻的精确版本；**非 symlink**] []
...
- **上游 checkout @ `dsh-v0.1.5-rc.2` 仍为 dev base**（2026-09-14 `pnpm install && pnpm run build` 全绿，约 1.8G）：`upstream/deepseek-harness/apps/cli/lib/bin.js` 即 installation。**现役唯一 `@deepseek-ai` scope = `.tests/profiles/node_modules/@deepseek-ai/*`**（仓根 farm 已删；2026-09-14 的双 scope 态就此收编）。参考源 dsh-omp（OMP adapter 蓝本）与 dashr 仓在同机 `~/workspaces/`。
